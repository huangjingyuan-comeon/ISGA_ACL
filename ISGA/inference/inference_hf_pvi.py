...
if args.use_vllm:
    ...
    # do_sample = True,
    # num_beams = 1,
    # force_words_ids = [[29871, 30848], [29871, 32530]]
            ...
            if args.use_vllm:
                ...
                for index, example in enumerate(examples):
                    ...
                    if args.guidance_scale == 1:
                        if not args.speculative_sampling:
                            # print("===============1")
                            # predict_words = tokenizer(["真","假"], add_prefix_space=True, add_special_tokens=False).input_ids
                            # print(predict_words)
                            generation_output = model.generate(
                                input_ids=inputs["input_ids"].to(device),
                                attention_mask=inputs['attention_mask'].to(device),
                    ...
                    print(f"Input: {example}\n")
                    print(f"Output: {response}\n")
                    # print(f"All_output：{generation_output['scores'][0]}\n")
                    ...
                    #     prediction.append(token_prob)
                    #     # print(f"Token ID: {token_id}, Probability: {token_prob}")
                    # print(prediction[-1])
                    # sent_prob = prediction[-1]
                    # results.append({"Input": input_text, "Output": response, "Prediction": prediction[-1]})


                    probs = torch.stack(generation_output['scores'], dim=1).softmax(-1)
                    generated_ids = generation_output['sequences'].tolist()
                    sentence = tokenizer.decode(generated_ids[0], skip_special_tokens=True)
                    sent_prob = probs[0][0][generated_ids[0][1]].item()
                    print(sent_prob)

                    results.append({"Input": input_text, "Output": response, "Prediction": sent_prob})
